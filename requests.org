...
** DONE [#A] inserting smart-punctuation when there's already a space between the word and the point 
...
** DONE [#A] [[file:shared-functions.org::*erika-send-email-styled][erika-send-email-styled]] (I changed my setup, which broke this function. See my note in the other file)
** PLEASE-CLARIFY [#B] [[file:shared-functions.org::*new-email-from-subtree][new-email-from-subree]]
** PLEASE-TEST other smart-punctuation behaviors below 
...
*** TODO change smart-return behavior for headings
Make org-return insert line break, not separate headings from stars

In the following example, ignore the commas. I just put them in so org doesn't think these are actual headings. ^ is the cursor.
...
*** TODO no autocapitalize checklist items
- [ ] This
- [ ] That
- [ ] And this 

These get autocapitalized... can we disable?

*** TODO capitalize after ellipsis and line break
...
**** DONE [#A] smart-punctuation when there's already a misplaced punctuation mark?
...
**** DONE [#A] smart-punctuation when there's already a correctly placed punctuation mark
Example:
...
*** capitalize after ellipsis and line breaks
...
*** an edge case for kill-word-correctly-and-capitalize 
