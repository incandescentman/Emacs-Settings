...
* DONE capitalize first character of new line after ellipsis
...
* DONE Change "defadvice capitalize-word" so that capitalize word doesn't downcase past a line break 
...
How to reproduce the error: instructions in the new file downcase-error-example-file.org
...
* TODO kill to buffer end 
See the function I created [[file:shared-functions.org::*kill%20to%20buffer%20end][kill-to-buffer-end]] and the keybinding I created for it. Can we make it so that this keybinding applies only in message-mode and not other modes?
