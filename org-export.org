...
(defconst jay/org-latex-emoji-regexp "[🀀-🫿]"
  "Regexp matching high-plane emoji that need LaTeX export handling.")
...
Org's `\\n:t' export option can append `\\\\[0pt]' after raw LaTeX command
lines and environments. That is useful in prose, but invalid after commands such
as `\\thispagestyle' or `\\end{center}'."
  (when (org-export-derived-backend-p backend 'latex)
    (with-temp-buffer
      (insert text)
      (goto-char (point-min))
