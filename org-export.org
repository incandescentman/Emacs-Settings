...
(defun jay/org-export-disable-smart-quotes-for-markdown (plist backend)
  "Use straight quotes for Markdown exports while preserving smart quotes elsewhere."
  (if (org-export-derived-backend-p backend 'md)
      (plist-put plist :with-smart-quotes nil)
    plist))

(add-to-list 'org-export-filter-options-functions
             #'jay/org-export-disable-smart-quotes-for-markdown)
...
*** Change section numbering depending on export format

#+begin_src emacs-lisp
(defun my-org-export-change-options (plist backend)
  "Disable section numbers for HTML and enable them for LaTeX exports."
  (cond
...
** default org-css
#+begin_SRC emacs-lisp
(defvar my-org-selected-css "/Users/jay/Dropbox/web-design/custom-css/sans-serif.css"
  "Path of the selected CSS file, or nil for no stylesheet. Defaults to sans-serif.css if not manually selected.")
...
(defvar my-org-css-options
  '("/Users/jay/Dropbox/web-design/custom-css/email.css"
    ...
    "/Users/jay/Dropbox/github/incandescentman/css/bold.css"
    "/Users/jay/Dropbox/web-design/custom-css/chatgpt-images.css"
    "/Users/jay/Dropbox/web-design/custom-css/sakura-1.50.css"
    "/Users/jay/Dropbox/web-design/custom-css/sans-serif.css"
    "/Users/jay/Dropbox/github/incandescentman/css/iphone.css"
    "none")
...
(defun select-org-export-css ()
  ...
  (let ((selection
         (completing-read "Choose a CSS file or none: "
                          my-org-css-options nil t)))
    (setq my-org-selected-css
          (if (string-equal selection "none")
              nil
            (unless (file-readable-p selection)
              (user-error "CSS file is not readable: %s" selection))
            selection))))
...
(defun my-org-inline-css-hook (exporter)
  ...
  (when (eq exporter 'html)
    ...
    (cond
     ((null my-org-selected-css)
      (setq org-html-head-include-default-style nil
            org-html-head nil))
     ((not (file-readable-p my-org-selected-css))
      (user-error "Selected CSS file is not readable: %s"
                  my-org-selected-css))
     (t
      (setq org-html-head-include-default-style nil
            org-html-head
            (concat
             ...
             "<!--/*--><![CDATA[/*><!--*/\n"
             (with-temp-buffer
               (insert-file-contents my-org-selected-css)
               (buffer-string))
             "/*]]>*/-->\n"
             "</style>\n"))))))
...
*** Preserve the unsubscribe URL template placeholder
#+begin_src emacs-lisp
(defun jay/org-export-preserve-unsubscribe-url (backend)
  "Preserve the exact {{unsubscribe_url}} placeholder in HTML link targets.

Org otherwise treats `{{unsubscribe_url}}' as an unresolved fuzzy link and
aborts the export.  Rewrite only that exact target to a private link type in
the temporary export buffer; all other unresolved links retain Org's normal
error behavior."
  (when (org-export-derived-backend-p backend 'html)
    (save-excursion
      (goto-char (point-min))
      (while (search-forward "[[{{unsubscribe_url}}][" nil t)
        (replace-match "[[jay-unsubscribe-url:placeholder][" t t)))))

(org-link-set-parameters
 "jay-unsubscribe-url"
 :export (lambda (_path description backend _info)
           (if (org-export-derived-backend-p backend 'html)
               (format "<a href=\"{{unsubscribe_url}}\">%s</a>"
                       (or description "{{unsubscribe_url}}"))
             (or description "{{unsubscribe_url}}"))))

(add-hook 'org-export-before-parsing-functions
          #'jay/org-export-preserve-unsubscribe-url)
#+end_src
...
(defun jay/org-export-id-link-replacements ()
  "Collect safe text replacements for ID links in the export buffer.
Described links become their descriptions.  Bare links retain their identifier
path instead of failing export or silently discarding the visible information.
The returned replacements are ordered from the end of the buffer backward."
  (let (replacements)
          ...
          (let* ((contents-begin (org-element-property :contents-begin link))
                 ...
                 (end
                  (save-excursion
                    ...
                    (point))))
            (when replacement
              (push (list begin end replacement) replacements))))))
    replacements))

(defun org-export-id-link-removal (_backend)
  "Replace ID links with plain text in Org's temporary export buffer."
  (let ((replacements (jay/org-export-id-link-replacements)))
    (when replacements
...
(defun jay-load-latex ()
  ...
  (let ((base "/Users/jay/emacs/emacs-settings/latex-templates/")
        (files '("elegant-helvetica.el"
                 "beautiful-racket.el"
                 "elegant-garamond.el"
                 "socratic-proposal.el"
                 "thought-leadership-playbook.el"
                 "thought-leadership-playbook-warm.el"
...
(defvar jay/org-latex-class-template-files
  '(("elegant-helvetica" . "/Users/jay/emacs/emacs-settings/latex-templates/elegant-helvetica.el")
    ...
    ("socratic-proposal" . "/Users/jay/emacs/emacs-settings/latex-templates/socratic-proposal.el")
    ("thought-leadership-playbook" . "/Users/jay/emacs/emacs-settings/latex-templates/thought-leadership-playbook.el")
    ("thought-leadership-playbook-warm" . "/Users/jay/emacs/emacs-settings/latex-templates/thought-leadership-playbook-warm.el")
...
the current buffer has no explicit #+EXPORT_FILE_NAME and has a
non-empty #+TITLE:, rename the exported PDF in place (overwriting any
existing target) and return the new path.  Preserve PDF-FILE when an
explicit export filename is present, when no title is available, or
when the return value is not a PDF path (async export returns a process,
not a filename)."
  (if (and (stringp pdf-file)
           (string-suffix-p ".pdf" pdf-file)
           (file-exists-p pdf-file))
      (let* ((explicit-name
              (cadr (assoc "EXPORT_FILE_NAME"
                           (org-collect-keywords '("EXPORT_FILE_NAME")))))
             (title (unless (and explicit-name
                                 (not (string-empty-p
                                       (string-trim explicit-name))))
                      (jay/org-export-title-for-filename)))
