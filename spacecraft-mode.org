...
** fill and unfill helpers
#+BEGIN_SRC emacs-lisp
(defconst jay/org-unfill-protected-labels '("HED" "SUBHED")
  "Org paragraph labels that `jay/unfill-paragraph-dwim' must not merge.")

(defun jay/org-unfill-protected-paragraph-p ()
  "Return non-nil when point is in a protected Org label paragraph.
This keeps title-label runs like \"HED:\" and \"SUBHED:\" on separate
lines instead of merging them into one paragraph."
      ...
      (when (eq (org-element-type element) 'paragraph)
        (save-excursion
          ...
          (let ((end (org-element-end element))
                (label-regexp
                 (concat "[ \t]*"
                         (regexp-opt jay/org-unfill-protected-labels t)
                         ":[ \t]*\\S-"))
                saw-line
                all-protected)
            (setq all-protected t)
            (while (and all-protected (< (point) end))
              (unless (looking-at-p "[ \t]*$")
                (setq saw-line t)
                (setq all-protected (looking-at-p label-regexp)))
              (forward-line 1))
            (and saw-line all-protected)))))))

(defun jay/org-unfill-block-delimiter-paragraph-p ()
  "Return non-nil when the current Org paragraph contains a block delimiter.
Org does not recognize same-type nested blocks, so an inner ~#+begin_*~
or outer ~#+end_*~ line can appear inside a paragraph element.  Skipping
that paragraph keeps unfill from joining the delimiter to prose."
  (let ((element (org-element-at-point))
        (case-fold-search t))
    (when (eq (org-element-type element) 'paragraph)
      (save-excursion
...
(defun jay/org-unfill-paragraph-element ()
  "Unfill the current Org paragraph while preserving Org hard line breaks.
Unlike `org-fill-paragraph', this directly unwraps ordinary prose
paragraphs, but it leaves protected title-label paragraphs alone."
  (let* ((element (save-excursion (end-of-line) (org-element-at-point)))
    ...
    (when (eq type 'paragraph)
      (let ((beg (max (point-min)
        ...
        (unless (or (< (line-end-position) beg)
                    (jay/org-unfill-protected-paragraph-p)
                    (jay/org-unfill-block-delimiter-paragraph-p))
          (save-excursion
            (goto-char beg)
            (let ((cuts (list beg)))
              ...
              (dolist (c (delq end cuts))
                (fill-region-as-paragraph c end)
                (setq end c)))))))))

(defun jay/org-unfill-element ()
  "Unfill the current Org paragraph without changing structural elements.
Paragraphs use `fill-region-as-paragraph' so wrapped prose really
unwraps.  Source blocks, drawers, block delimiters, and every other
non-paragraph element are left byte-for-byte alone."
  (let ((element (save-excursion (end-of-line) (org-element-at-point))))
    (when (eq (org-element-type element) 'paragraph)
      (jay/org-unfill-paragraph-element))))

(defun jay/org-unfill-region-preserving-structure ()
  "Unfill each Org paragraph intersecting the active region.
Protected title-label paragraphs like \"HED:\" and \"SUBHED:\" are
skipped so they do not collapse onto one line.  Paragraph positions are
collected from Org's syntax tree first and processed from end to start,
so source blocks and other structural elements are never passed to fill
machinery and earlier paragraph positions stay valid as text shrinks."
          ...
          (org-element-map (org-element-parse-buffer) 'paragraph
            (lambda (element)
              (let ((beg (org-element-begin element))
                    (finish (org-element-end element)))
                (when (and (< beg end) (> finish start))
                  beg))))))
    (unwind-protect
        (dolist (position (sort paragraphs #'>))
          (goto-char position)
          (jay/org-unfill-paragraph-element))
      (goto-char origin)
      (set-marker origin nil))))

(defun jay/unfill-paragraph-dwim ()
  "Unfill prose while preserving Org structure.
In Org buffers, unwrap ordinary prose paragraphs directly while still
leaving drawers, planning lines, tables, source blocks, block delimiters,
and other non-paragraph structure unchanged.  With an active region in
Org mode, unfill only paragraph elements in the region.  Protected title-label
paragraphs like \"HED:\" and \"SUBHED:\" are skipped."
  (interactive)
  (let ((fill-column most-positive-fixnum))
    (cond
     ((and (derived-mode-p 'org-mode) (use-region-p))
      (jay/org-unfill-region-preserving-structure))
     ((derived-mode-p 'org-mode)
      (jay/org-unfill-element))
     ((use-region-p)
...
(defun my/sentence-tail-info ()
  "Return info about the tail of the current sentence from point.
The return value is a plist with:
- =:end=: the exclusive end position for killing the sentence tail
- =:terminal=: the sentence-ending punctuation character to preserve, or nil

This keeps paragraph breaks intact by excluding trailing whitespace
after the sentence-ending text. If no sentence-ending punctuation is
found, the tail ends at end of line."
  (save-excursion
    (let ((limit (save-excursion (end-of-line) (point))))
      (if (re-search-forward "[.?!][]\"'”’)}]*" limit t)
          (list
           :end (match-end 0)
           :terminal (char-after (match-beginning 0)))
        (list
         :end limit
         :terminal nil)))))

(defun my/delete-horizontal-space-around-point ()
  "Delete spaces and tabs around point, but preserve newlines."
  (let (beg end)
    (save-excursion
      (skip-chars-backward " \t")
      (setq beg (point)))
...
sentence. Otherwise kill forward but preserve any punctuation at the sentence end."
  ...
  (if (my/beginning-of-sentence-p)
    ...
    (let* ((info (my/sentence-tail-info))
           (end (plist-get info :end))
           (terminal (plist-get info :terminal)))
      (kill-region (point) end)
      (my/delete-horizontal-space-around-point)
      (when (looking-back "[,;:]" (max (point-min) (1- (point))))
        (delete-backward-char 1))
      (when terminal
        (insert-char terminal)
        (unless (or (eobp)
                    (looking-at "[ \t\n]"))
          (insert " ")))))
...
(defun kill-word-correctly ()
  ...
  (if (or (re-search-forward "\\=[ \t]*\n" nil t)
          (re-search-forward "\\=\\W*?[[:punct:]]+" nil t))
      (kill-region (match-beginning 0) (match-end 0))
    (kill-word 1))
  (my/fix-space)
  ;; don't leave two commas in a row
  (when (or
         (looking-at "\\,\\, ")
         (and
          (looking-at "\\,")
...
** capitalize-word advice
#+BEGIN_SRC emacs-lisp
(defun jay/capitalize-word-then-downcase-next (&rest _)
...
certain words/contexts (like \"I\")."
  (unless (or
           ...
           (looking-at "\")[[:space:]]*$")       ; quote + right paren, optional whitespace to EOL
    ...
    (save-excursion
      (downcase-word 1))))
(advice-add 'capitalize-word :after #'jay/capitalize-word-then-downcase-next)
#+END_SRC
