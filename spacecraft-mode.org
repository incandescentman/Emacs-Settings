...
(defun auto-capitalize--inserted-non-word-p (beg end length)
  "Return non-nil if this command inserted a non-word character.
Checks whether `this-command' is `self-insert-command' for a non-word char,
or is a newline command."
  (and (eq this-command 'self-insert-command)
       (= length 0) ;; no deletion in this change
       (= (- end beg) 1) ;; exactly 1 char inserted
       (let ((ch (char-before end)))
         (not (eq (char-syntax ch) ?w))))
   ;; Alternatively, treat newlines as well:
