...
(defun jay/org-unfill-element ()
  "Unfill the current Org element with direct prose handling.
Paragraphs use `fill-region-as-paragraph' so wrapped prose really
unwraps, while other elements defer to Org's own fill machinery."
  (let ((element (save-excursion (end-of-line) (org-element-at-point))))
...
(defun jay/org-unfill-region-preserving-structure ()
  "Unfill each fillable Org element in the active region.
Protected title-label paragraphs like \"HED:\" and \"SUBHED:\" are
skipped so they do not collapse onto one line."
...
letting Org preserve drawers, planning lines, tables, source blocks,
and other non-paragraph structure.  With an active region in Org mode,
unfill each fillable element in the region.  Protected title-label
