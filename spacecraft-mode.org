    ...
    (unwind-protect
        (progn
          ...
          (let ((org--single-lines-list-is-paragraph nil))
            (while (> (point) start)
              (unless (jay/org-unfill-protected-paragraph-p)
                (org-fill-element))
...
In Org buffers, use `org-fill-paragraph' with a huge `fill-column' so
Org itself decides which elements count as fillable prose.  That keeps
drawers, property drawers, planning lines, tables, and other
non-paragraph structure intact.  With an active region in Org mode,
