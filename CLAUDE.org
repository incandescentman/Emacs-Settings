...
- =org-roam-config-legacy.el= - Legacy entry point (delegates to suite)
...
6. =org-authoring.el=
7. =spellcheck.el=
8. =shared-functions.el=
9. Various feature modules (deferred or immediate)
10. Local configuration (=local-config.el=)
...
- *Legacy Cleanup*: Deprecated =org-roam-config-legacy.el= to a compatibility shim pointing to =jay-org-roam-suite/=.
