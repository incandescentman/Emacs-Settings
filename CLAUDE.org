...
- *LaTeX hyphenation preference*: Jay dislikes hyphenation in prose output. Prose classes inherit a hyphenation-averse baseline from [[file+emacs:/Users/jay/emacs/emacs-settings/latex-templates/jay-latex-prose-defaults.el][latex-templates/jay-latex-prose-defaults.el]] — currently =\finalhyphendemerits=1000000= (surgical anti-runt) + =\setlist{before=\RaggedRight}= (bullets ragged-right). When fixing hyphenation/line-break issues, diagnose the specific failure pattern and match a narrow TeX knob to it (=\finalhyphendemerits=, =\righthyphenmin=, =\setlist{before=\RaggedRight}=) rather than reaching for blunt globals like =\sloppy= or =\hyphenpenalty=10000= — blunt globals produce loose justified lines. Never add =\exhyphenpenalty=10000= — it blocks breaks at explicit hyphens like "well-known," which should stay breakable. Anything added to =jay/latex-prose-defaults= must also go in =jay/latex--strip-prose-defaults= to keep the helper idempotent.
