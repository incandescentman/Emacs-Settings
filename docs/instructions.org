...
- =spacemacs.d/init.el= - Main Spacemacs configuration (=/Users/jay/emacs/emacs-settings/spacemacs.d/init.el=)
- =shared-functions.org= - Most custom functions (9,983 lines, auto-tangles to =.el=)
- =pasteboard-copy-and-paste-functions.el= - Clipboard magic (pure elisp, edit directly)
- =fonts-and-themes.org= - Visual appearance
- =spacemacs.d/= - Spacemacs config directory (symlinked from =~/.spacemacs.d=)
...
Note: =pasteboard-copy-and-paste-functions.el= is pure elisp and does not tangle.
...
Custom LaTeX templates are in =latex-templates/=. Use them by adding =#+LATEX_CLASS: template-name= to your org file.
...
2. Check =spacemacs.d/init.el= for unused layers (comment them out)
...
Spacemacs keybindings: edit =spacemacs.d/init.el= in the =dotspacemacs/user-config= section.
...
| File                                     | Purpose                          |
|------------------------------------------+----------------------------------|
| =spacemacs.d/init.el=                    | Spacemacs main config            |
| =shared-functions.org=                   | Most custom functions            |
| =pasteboard-copy-and-paste-functions.el= | Clipboard magic                  |
