...
| Step | When                             | Action                                                    |
|------+----------------------------------+-----------------------------------------------------------|
|    1 | Start here                       | =/Applications/Emacs.app/Contents/MacOS/Emacs --debug-init= |
|    2 | Error is in your config files    | Temporarily rename config                                 |
|    3 | "package not found" / corruption | =rm -rf ~/.emacs.d/elpa=                                    |
|    4 | Spacemacs itself is broken       | =/Applications/Emacs.app/Contents/MacOS/Emacs -Q= or =emacs-min=  |
...
*Why these use explicit paths*: Your =em= alias uses =open -a= which can't pass flags like =--debug-init=. For debug/vanilla runs, use the Emacs.app binary directly.

** Direct Commands (no alias required)
#+begin_src sh
/Applications/Emacs.app/Contents/MacOS/Emacs --debug-init
...
#+begin_src sh
/Applications/Emacs.app/Contents/MacOS/Emacs -Q  # Emacs with zero config
