...
1. Run =emacs-debug= in Terminal
2. Read the error in the backtrace
3. Match it to the Quick Reference table → pick the fix
4. If you just need Emacs NOW: run =emacs-known-good=
...
alias emacs-min='/Applications/Emacs.app/Contents/MacOS/Emacs -Q -l /Users/jay/emacs/emacs-settings/archive/minimal-init.el'
#+end_src
...
emacs-min      # Emacs with archive/minimal-init.el
#+end_src
