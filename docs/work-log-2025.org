- *2025-12-26: Restore Org emphasis fontification with safe dash boundaries*
- Rebuilt Org emphasis regex after customizing =org-emphasis-regexp-components= in =spacemacs-new-config.el= so inline emphasis re-fontifies in prose buffers
- Allowed em/en dashes only on the pre-side of the emphasis boundary to avoid breaking fontification while supporting dash-adjacent emphasis

- *2025-12-03: Pasteboard markdown fence converter relocated and fixed*
- Moved `convert-markdown-to-org-code-blocks-simple` out of `shared-functions.org` into `pasteboard-copy-and-paste-functions.el` (source of truth), so paste-clean pipeline owns the helper it calls
- Rewrote the converter to toggle fences statefully (handles unlabeled ``` blocks) to stop the duplicated `#+end_src` output seen in Org pastes
- Retangled `shared-functions.org` after removing the stale definition and reran `check-parens` on both `shared-functions.el` and `pasteboard-copy-and-paste-functions.el` to confirm structure

- *2025-12-02: Goals system documentation cleanup*
- Updated =goals-agenda.org=: removed obsolete setup instructions (directory creation, first-time setup steps), kept elisp code and usage reference
- Fixed velocity paths in =goals-agenda.org=: projects dir now =~/Library/CloudStorage/Dropbox/github/velocity/projects/=, master task list now in =goals/= not =data/=
- Fixed keybindings in =velocity/goals-system/complete-user-guide.org=: changed =C-c a d= to =s-k a d= to match actual config
- Added cross-reference headers to both Jay-facing guides: =goals-instructions-for-Jay.org= (complete reference) and =complete-user-guide.org= (quick reference)

- *2025-11-29: Fix pasteboard markdown detection for inline bold*
- Added =\\*\\*[^*\n]+\\*\\*= pattern (weight 3) to =pasteboard--analyse-clipboard-text= for detecting inline =**bold**= markers
- Previously, text with =**bold**= patterns but no markdown headings wasn't detected as markdown style
- This fix ensures =pasteboard--convert-markdown-inline-emphasis= runs to convert =**bold**= → =*bold*=
- Also enables =normalize-dashes= to convert spaced hyphens (= - =) to em-dashes (= --- =)
...
- Captured the full investigation, commands, and open questions in `docs/info-system-debugging.org` for follow-up.
...
- New notes: `docs/info-system-debugging.org`, `info/dir`.
...
* 2025-12-16 Tuesday
- Reviewed and improved `shared-functions.org`.
- Added a descriptive docstring to the `jd-org-current-time` function.
- Deleted the obsolete `html2org-clipboard-and-unfill-paragraph` function.
- Refactored `defadvice isearch-exit` to modernize the code, moving logic into `my-goto-match-beginning` and removing the deprecated `defadvice` block.

* 2025-12-16 Tuesday (continued)
- Refactored `defadvice` blocks to `advice-add` for `recentf-save-list`, `configuration-layer/update-packages`, and `sp-show--pair-function`.
- Added/Updated docstrings for `new-buffer`, `new-lisp-buffer`, `org-new-scratch-buffer`, `conditionally-disable-abbrev`, and `cycle-hyphenation-or-toggle-item`.

* 2025-12-16 Tuesday (continued)
- Modernized `eval-after-load` to `with-eval-after-load` for `org-list` and `org-src`.

* 2025-12-16 Tuesday (continued)
- Modernized deprecated Org export variables: `org-export-latex-*` to `org-latex-*`. Removed obsolete `org-export-html-inline-image-extensions`.

* 2025-12-16 Tuesday (continued)
- Refactoring anonymous lambdas in hooks to named functions (`jay/disable-auto-fill`, `jay/disable-abbrev`) for better debugging and modularity.

* 2025-12-16 Tuesday (continued)
- Refactored anonymous lambdas to named functions (`jay/disable-auto-fill`, `jay/disable-abbrev`) in `shared-functions.org`.
