...
(setq version-control t          ; Use numbered backups
      kept-new-versions 9        ; Keep 9 newest
...
*What*: Dropbox syncs org-roam directories (=~/Dropbox/roam/=, etc.) but NOT =emacs-settings=.

*Note*: The =emacs-settings= repo is at =~/emacs/emacs-settings= (not in Dropbox). It relies on Git/GitHub for offsite backup, not Dropbox sync.
...
3. *Verify symlink*:
   #+BEGIN_SRC sh
   ls -l ~/.spacemacs.d
   # Should point to /Users/jay/emacs/emacs-settings/spacemacs.d
   #+END_SRC
...
#+BEGIN_SRC sh
# Backup config first
cd ~/emacs && tar czf emacs-settings-backup.tar.gz emacs-settings

# Delete packages and restart
