...
9. Loads shared-functions.el (tangled from shared-functions.org)
10. Loads other modules (deferred or immediate based on config)
11. Loads local-config.el (if exists, gitignored for machine-specific settings)
#+END_SRC
...
*If splitting ever becomes necessary*, start with low-coupling blocks:
- Keybindings + key-chord setup (move to =keys.el= / [[file+emacs:/Users/jay/emacs/emacs-settings/archive/key-chord.org][archive/key-chord.org]])
- Search/completion stack (isearch, swiper, consult, rg, deadgrep)
- Org-roam + org-roam-ui config stubs
- UI/appearance + font/frame logic (move to =fonts-and-themes.org=)
- Web research/html2org helpers (move near pasteboard module)
Note: Search/completion now lives in =search-commands.org= as the first extraction.
