...
`pasteboard--clean-string` processes text in this order (ORDER MATTERS):
  ...
  5. `convert-markdown-blockquotes-to-org` (strips `>` markers from single- and multi-line blockquotes, preserving blank lines)
  6. `convert-markdown-links-to-org-mode`
  7. Narrowed pass: `asterisk-to-dash-and-convert-code-blocks-to-org` (if available)
  8. Markdown table cleanup: `pasteboard--tighten-markdown-table-separators`
  9. Ensure a blank line before headings unless the previous line is also a heading (keeps `*`/`**` sequences tight)
