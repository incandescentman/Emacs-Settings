...
** Files Modified
- `markdown-diary.el`: Replaced the trailing-newline helper with a blank-line inserter and bound `c` to the entry command.
- `docs/calendar-keybindings.org`: Documented the new `c` shortcut.
