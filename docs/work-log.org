...
* 2025-10-19: Universal argument support for adaptive copy/paste

** Summary
- Added prefix argument (C-u) support to `pasteboard-copy-adaptive` and `pasteboard-paste-adaptive`
- C-u s-c now forces verbatim copy, overriding smart context detection
- C-u s-v now forces verbatim paste, overriding smart context detection
- Normal s-c and s-v continue with existing smart behavior

** Files Modified
- `pasteboard-copy-and-paste-functions.org`: Added `current-prefix-arg` checks to both functions with early-return verbatim paths
