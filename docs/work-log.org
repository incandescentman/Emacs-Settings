...
** Files Modified
- `markdown-diary.el`: Implemented the help popup, toggle command, and which-key integration.
- `docs/calendar-keybindings.org`: Documented the new ~?~ and ~M-t~ bindings.
- `markdown-diary-README.org`: Refreshed the cheat-sheet table and noted the optional which-key hints.
...
- Added a `?` cheat-sheet popup, a `M-t` today/last-date toggle, and friendly which-key labels so navigation stays discoverable.
...
The help popup and which-key hints reuse existing keybindings for reference, and the toggle stores the last date so the jump is fully reversible.
