...
- Introduced =jay/org-format-heading-spacing= in =shared-functions.org= to call the formatter on the current buffer's file: saves if modified, shells out to Python, and reverts on success.
