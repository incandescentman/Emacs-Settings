...
* 2025-10-14: Calendar diary prompt variants and spacing fixes
...
** Files Modified
- `markdown-diary.el`: Added `my-calendar-insert-diary-entry-and-autopopulate`, shared helper, and spacing normalisers.
- `docs/calendar-keybindings.org`: Described the `c`/`C` split and the prefix behaviour.
...
** Files Modified
- `markdown-diary.el`: Replaced the trailing-newline helper with a blank-line inserter, added the `c` alias, introduced `my-calendar-open-diary-entry`, retuned `RET`, and improved `e` to place point at the last bullet.
...
- Consider a quick `emacs --batch` smoke test that exercises `my-calendar-open-diary-entry` and `my-calendar-view-fancy-listing`.
