...
* 2025-11-08: Keep org-roam on direct Dropbox mount
...
** Verification
- `rg -n "file-truename .*Dropbox/roam" -g'*'` (no hits in active config)
- Restart Emacs and run `M-x jay/org-roam-db-sync` to confirm autosync uses `/Users/jay/Dropbox/roam`

** Impact
- org-roam no longer attempts to create directories inside `/Users/jay/Library/CloudStorage`
- Autosync, idle timers, and profile switches all operate against `/Users/jay/Dropbox/roam`, eliminating the timeout spam
- Future profile additions inherit the safer directory handling automatically
...
** Verification
- Searched for remaining old paths: none found in active config files
- Ran check-parens on tangled elisp: passed
- All paths now point to `/Users/jay/Dropbox/` instead of `/Users/jay/Library/CloudStorage/Dropbox/`
