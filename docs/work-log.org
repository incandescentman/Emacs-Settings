...
** Files Modified
- `shared-functions.org`: Bound new month/year navigation lambdas in `calendar-mode-map`.
- `emacs-calendar-instructions-claude.org`: Documented the updated keybindings, including Option-arrow aliases.
- `docs/work-log.org`: Added this entry.
