...
* Overview
This document lists custom keybindings that use the "s-" prefix (macOS Command key) from the configuration.
Source of truth: [[file:../keys.el][keys.el]].
...
* Org-mode overrides
These are defined in the org-mode block in `keys.el` and only apply in Org buffers:
- ~s-v~ → ~pasteboard-paste-adaptive~
- ~s-l~ → ~org-insert-link~
