...
** [PLANNED] Create "Known Good" Recovery System

*Why it matters*:
- If a config change breaks Emacs, no easy way to revert
- Manual git checkout requires remembering which commit worked
- Should be automatic and foolproof

*Proposal*:
Create =safe-emacs= wrapper script that:
1. Launches Emacs with timeout check (~10 seconds)
...
# Try to launch Emacs, timeout after 10s to check if it stays up
timeout 10 emacs "$@" &
PID=$!
sleep 10
...
*Also add*:
- Alias =emacs= to =safe-emacs= in shell config
- Document in =recovery-guide.org=
