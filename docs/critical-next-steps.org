...
4. The strongest specific candidates inside that file are =projectile-mode= and duplicate global Smartparens setup. Flycheck has already been narrowed from global activation to mode-scoped hooks for =prog-mode= and =conf-mode=.
