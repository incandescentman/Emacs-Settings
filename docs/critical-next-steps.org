...
*Reason*: Significant rewrite of core documentation files (`instructions.org`, `design-architecture.org`, etc.) to improve clarity, comprehensiveness, and user-friendliness. Updated `CLAUDE.md` and `README.org` to reflect codebase changes and new architectural components (e.g., Org Roam profiles, LaTeX templates).
