...
;; (org-or-orgalist-p)
  ...
  (defun jay/org-checkbox-toggle-and-advance ()
    "Tick the checkbox at point and move on to the next list item.

When point is not on a checkbox item, fall through to `org-todo',
so that one key means \"mark this thing done and move on\" for both
checkbox items and TODO headings.

The advance itself is performed by `jay/org-checkbox-advance', via the
`:around' advice on `org-toggle-checkbox' in shared-functions.org.  Do
not add an explicit move here — that would advance twice."
    (interactive)
  ...
  (defun jay/org-next-checkbox ()
    "Move to the next checkbox item without changing any checkbox state.

Non-checkbox list items are skipped.  If there is no later checkbox in
the current list, leave point where it started.  Point lands inside the
checkbox marker."
    (interactive)
    (let ((start (point)))
      (condition-case nil
          (progn
            (org-next-item)
            (while (not (org-at-item-checkbox-p))
              (org-next-item))
            (beginning-of-line)
            (search-forward "[" (line-end-position) t))
        (error (goto-char start)))))

  (defun jay/org-next-checkbox-or-insert-link ()
    "Advance from a checkbox, or otherwise insert an Org link."
    (interactive)
