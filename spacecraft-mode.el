...
(defun my-sentence-start-with-bullets ()
  "Return the start of a sentence by punctuation or beginning of line.
If this line starts with a bullet (-, +, *), return nil (no auto-cap)."
  (save-excursion
    ;; Try to find punctuation + whitespace going backward:
    (if (re-search-backward "[.?!]\\s-+" nil t)
        (progn
          ;; Skip the punctuation & whitespace so we're right at the
          ;; first character of the new "sentence."
          (skip-chars-forward ".?! \t\r\n")
          (point))
      ;; Otherwise, fallback to the beginning of the line:
      (beginning-of-line)

      ;; If the line is a bullet, return nil so we skip capitalization:
      (if (looking-at-p "^[ \t]*[-+*][ \t]+")
          nil
