...
~a~   ~my-timeline-upcoming~
     - Open the read-only *Timeline Upcoming* buffer listing future-dated
       entries in chronological order (weekday + days-until). A numeric prefix
       caps the count; ~C-u a~ shows all. Inside that buffer, ~RET~/~o~ jumps to
       the date in the Calendar, ~g~ refreshes, ~q~ quits.
