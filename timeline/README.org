...
* Stacked Calendar and Itinerary

Opening Calendar saves the current writing layout and places a short three-month grid above a full-width, read-only Org itinerary. The itinerary follows the selected date, showing a week before and three weeks after it, with ongoing multi-day events included. ~RET~/~e~ swaps the itinerary for the real diary editor; following pauses until ~C-c C-c~ returns. ~q~ in Calendar restores the original layout, except that an active diary editor is kept visible.

- ~j~ and ~g d~ use Org's natural-language date prompt; the rest of the ~g~ prefix remains intact.
- ~V~ toggles following and remembers the preference through Custom. ~v~/~o~/~SPC~ show the selected date manually when following is off.
- ~C-c <left>~/~C-c <right>~ go back and forward through deliberate jumps without recording arrow movement.
- In the itinerary, ~<~/~>~ extend earlier/later by four weeks; page scrolling extends at the boundaries. ~g~ refreshes and ~q~ returns focus to the grid.
- ~s-k c c~ (~M-x my-timeline-capture~) captures a date and event from any buffer, then returns to the current work. Capture leaves preexisting unsaved diary edits unsaved.
- ~?~ displays an Org guide whose displayed shortcuts come from the current keymaps.

Upcoming and the itinerary share one buffer-setup helper and a cache of entries and paired ranges keyed by the live source buffer's character modification tick. Browsing preserves the diary's point, narrowing, text, and modified state. Olivetti is disabled locally in the generated views; their Org mode is initialized once and their buffers are reused. The Markdown diary and Timeless range format remain the shared source of truth.
...
- 🧭 *Smart Navigation & Focus Control*:
 ...
 - ~v~, ~o~, or ~SPC~ --- Show the itinerary while keeping focus in the calendar.
...
- 🔭 *Upcoming Events View*: ~a~ opens =my-timeline-upcoming=, a read-only "what's next" buffer grouped into Today, This week, and Later. Paired multi-day boundaries collapse into one item with range-aware status such as ~Starts today~, ~Ongoing · ends tomorrow~, or ~Last day today~; ~RET~/~o~ jumps to the entry or range start in the Calendar.
...
| ~v~, ~o~, ~SPC~ | =my-calendar-view-diary-entry= | Show itinerary but stay in calendar |
...
| ~a~ | =my-timeline-upcoming= | List upcoming events with range-aware sections and status |
...
- ~/~ triggers =my-diary-search= for diary-wide searching; ~s~ remains available for typing.
...
- ~my-timeline-diary-file~ is a customizable file option; changing it keeps Emacs' stock ~diary-file~ synchronized.
- Multi-day range pairing is presentation-only: the Markdown file retains the documented first-day/last-day protocol, while the upcoming view collapses paired boundaries and leaves unpaired final markers visible as ordinary entries.
...
- [[file+emacs:/Users/jay/emacs/emacs-settings/timeline/timeline-agenda.el][timeline-agenda.el]] — shared cached data and Org view setup, the following itinerary, and Upcoming.
- [[file+emacs:/Users/jay/emacs/emacs-settings/timeline/timeline-tests.el][timeline-tests.el]] — regression coverage for insertion, cleanup, capture, range rendering, cache invalidation, navigation, and window behavior.
- ~timeline.el~ — entry point that stitches the modules together and installs the shared keybindings.

The current workflow and implementation contracts are maintained in [[file+emacs:/Users/jay/emacs/emacs-settings/docs/emacs-calendar-instructions.org][the Calendar and Timeline guide]].
...
* Installation
1. Place the entire [[file+emacs:/Users/jay/emacs/emacs-settings/timeline/][Timeline module directory]]---the [[file+emacs:/Users/jay/emacs/emacs-settings/timeline/timeline.el][timeline.el entry point]] and its sibling modules---in your ~load-path~.
2. Optionally set a different Markdown diary before loading the module:
   #+begin_src emacs-lisp
   (setq my-timeline-diary-file "/path/to/timeline.md")
   #+end_src
3. Add to your init file:
   #+begin_src emacs-lisp
   (require 'timeline)
   #+end_src
4. Open the Calendar with ~M-x calendar~ and use the keybindings above.
