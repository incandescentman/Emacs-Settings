...
*** The {{{tl}}} package
Most standard LaTeX distributions ship an older version of the {{{tl}}} package. Tufte Org Mode requires version 3.5.2 of the package, which at the time of this writing was the version available from [[https://tufte-latex.github.io/tufte-latex/][the Tufte-LaTeX web site]]. In particular, the {{{tl}}} package must support the ~nobib~ option, which instructs the package not to load ~natbib~ support. The ~natbib~ support in older versions of the {{{tl}}} package clashed with ~biblatex~, which is very useful in a Tufte-style document.
