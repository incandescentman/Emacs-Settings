...
(defun pasteboard-copy-adaptive ()
  ...
  (interactive)
  (let (choice)                               ; record which branch we took
    (cond
     ;; ------------------------------------------ verbatim buckets ------------------------------------------
     ;; 1) Messages buffer - ALWAYS verbatim
     ((string= (buffer-name) "*Messages*")
      (setq choice "verbatim (Messages buffer)")
      ...
      (if (and (use-region-p)
               (save-excursion
                 ...
                 (looking-at-p
                  "\\s-*\\([({[]\\|[#;]\\|https?://\\)")))
          (progn
            (setq choice "verbatim (heuristic)")
            (call-interactively #'pasteboard-copy-verbatim))
        (setq choice "clean (default)")
        (call-interactively
...
In org-roam buffers, automatically demote headings by one level."
  ...
  (let* ((raw-clip (string-trim (shell-command-to-string "pbpaste")))
    ...
    ;; Now point is at the end of the pasted content
    (let ((paste-end-pos (point)))
      ;; If we're in an org-roam buffer and pasted org content, demote headings
      (when (and is-org-roam
                 (not (string= choice "bracket-link"))
                 (not (string= choice "verbatim"))
                 (> paste-end-pos paste-start-pos))
        (save-excursion
          (goto-char paste-start-pos)
          ;; Check if there are any headings to demote
          (when (re-search-forward "^\\*+ " paste-end-pos t)
            (goto-char paste-start-pos)
            (demote-org-headings-in-region paste-start-pos paste-end-pos)
            (setq choice (concat choice " (org-roam demoted)")))))
...
(defun pasteboard-paste ()
  ...
  (let ((start (point))
    ...
    (delete-region start end)
    (insert ins-text)
    (let ((paste-end (point)))  ; Save where the paste ended
      (my/fix-space)
      (save-excursion
        (goto-char start)
        (my/fix-space))
      (goto-char paste-end)))  ; Ensure point is at the end of pasted text
                                        ; (reflash-indentation)
  )
#+END_SRC
...
Otherwise run, in order:
  ...
  4. `asterisk-to-dash-and-convert-code-blocks-to-org`."
  (interactive "P")
...
;; (ensure-heading-emoji-space beg end)
        ...
        ;; Add the asterisk-to-dash conversion here
        (save-restriction
          (narrow-to-region beg end)
          (goto-char (point-min))
          (asterisk-to-dash-and-convert-code-blocks-to-org)
          (convert-markdown-headings-to-org (point-min) (point-max)))
        ;; Move point to end of pasted region
        (goto-char end)
