...
The replacements are defined in the `smart-quotes-replacement-pairs` variable."
  ...
  (let ((end-marker (copy-marker end)))
    (save-excursion
      ...
      ;; Skip if it's part of org heading stars at start of line
      (goto-char beg)
      (while (re-search-forward "\\*\\*" end-marker t)
        (let* ((match-pos (match-beginning 0))
               (line-start (line-beginning-position))
               ;; Check if ** is at start of line (part of heading)
               (at-line-start (= match-pos line-start)))
          (save-excursion
            (goto-char line-start)
            ;; Only skip if this ** is part of the heading stars at line start
            (if (and at-line-start (looking-at "^\\*+\\s-"))
                nil  ; Skip heading stars
  ...
  (let* ((clipboard-raw (pasteboard--clipboard-string))
    ...
    (cond
     ...
     ((or (and (eq major-mode 'org-mode)
      ...
      (let* ((cleaned (pasteboard--clean-string clipboard-raw))
             (final (if (and is-org-roam (string-match-p "^\\*+ " cleaned))
                        (progn
                          (setq choice "clean (org-roam demoted)")
                          (pasteboard--demote-headings-in-string cleaned))
                      (progn
                        (setq choice "clean")
                        cleaned))))
        (pasteboard-paste final)))
     (t
...
(defalias 'pasteboard-paste-raw #'pasteboard-paste-verbatim)
#+END_SRC
