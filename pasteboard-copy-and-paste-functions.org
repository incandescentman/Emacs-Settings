    ...
    (cond
     ...
     ((string= (buffer-name) "*Messages*")
      (setq choice "verbatim (Messages buffer)"
            handler #'pasteboard-copy-verbatim))
     ...
     ((or (eq major-mode 'text-mode)
      ...
      (setq choice "clean"
            handler #'pasteboard-copy-and-replace-em-dashes-in-clipboard))
    ...
    (unless (and skip-lines (memq line-no skip-lines))
      (if (> indent 0)
        ...
        (let ((prev-kind (pasteboard--neighbor-nonblank-kind pos 'prev))
          ...
          (and (not (memq prev-kind '(org-heading)))
               (not (memq next-kind '(org-heading)))
               (or (memq prev-kind '(asterisk-bullet dash-bullet ordered-bullet definition))
                   (memq next-kind '(asterisk-bullet dash-bullet ordered-bullet definition)))))))))
    ...
    (while (re-search-forward "^\\([ \t]*\\)\\* \\(.*\\)$" end t)
      (let* ((line-beg (match-beginning 0))
        ...
        (when (pasteboard--should-convert-asterisk-line line-beg indent skip-lines)
          (replace-match (concat indent-str "- " body) t t))))))
