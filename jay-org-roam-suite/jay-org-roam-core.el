...
;; Optional local fixes (loaded only if present beside this file).
(defun jay/org-roam--load-optional-fix (filename)
  "Load optional org-roam fix FILENAME from the suite directory."
  (when load-file-name
    (let* ((suite-dir (file-name-directory load-file-name))
           (suite-path (expand-file-name filename suite-dir)))
      (if (file-exists-p suite-path)
          (load suite-path t t)
        (message "org-roam: optional fix missing: %s (checked %s)"
                 filename suite-path)))))
...
(with-eval-after-load 'org
  (setq org-roam-db-autosync-mode nil
        ...
        org-roam-directory-exclude-regexp "^\\(archive\\|archive-excluded\\|documents\\)/"
        org-roam-node-display-template (concat "${title:*} " (propertize "${tags:15}" 'face 'org-tag))
        org-roam-dailies-directory "journal/"
        org-roam-file-exclude-regexp "\\.git/\\|attachments/\\|archive-excluded/\\|\\(?:^\\|/\\)archive/\\|\\.org~$\\|#.*#$\\|consumerist/fragrance-tracker/source/"
...
(global-set-key (kbd "s-u 4") #'jay/org-roam-switch-to-side-hustle)
(global-set-key (kbd "s-u 5") #'jay/org-roam-switch-to-parents)
...
(with-eval-after-load 'which-key
  (which-key-add-key-based-replacements
    ...
    "s-u 4"   "profile: Side Hustle"
    "s-u 5"   "profile: Parents"
