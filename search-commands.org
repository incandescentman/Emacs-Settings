...
[[file+emacs:/Users/jay/Dropbox/writing/both/proposal][📁 /Users/jay/Dropbox/writing/both/proposal]]

[[file+emacs:/Users/jay/Dropbox/writing/both/book][📁 /Users/jay/Dropbox/writing/both/book]]
...
- ~completing-read~ is then used to create an interactive selection interface
...
2. *Directory Definitions*: The helper functions resolve Jay's live proposal and book directories from a short list of expected Dropbox and CloudStorage paths.
3. *Listing Files*:
...
5. *Interactive File Selection*:
   - ~completing-read~ is used to create an interactive selection interface. It takes ~all-files~ as the list of choices to present to the user.
...
This function provides a user-friendly way to search and open files from two different directories using Emacs's current completion system, offering a seamless integration with your Emacs workflow.
...
* recent file picker (folder-first, aligned columns)
Folder-first display for =recentf-list=: the parent directory is shown
on the left in a shadow face, padded to a fixed-width column so that
all filenames line up vertically on the right.

*Architecture:* candidates are canonical abbreviated paths (like
=~/Dropbox/roam/foo.org=). Display is rendered via
=affixation-function= in the completion-table metadata, not by baking
spaces and faces into the candidate string. That keeps the seam
clean: Orderless matches on the path, Embark actions receive the
path, Marginalia can still annotate, and the visual layout stays the
affix-function's job.

=file-truename= resolves the =~/Library/CloudStorage/Dropbox/...=
symlink to =~/Dropbox/...=, so the two interchangeable macOS paths
collapse to a single canonical form automatically. Parent paths
longer than =jay/recent-file-parent-column-max= are truncated from
the *left* with a leading ellipsis, so the distinctive tail of the
path (the part closest to the file) stays visible.
...
* embark: extra file actions
Extends =embark-file-map= with a few Jay-specific file actions. They
apply to *any* file completion that declares =:category file= (plain
=find-file=, =consult-buffer='s file sources, =s-r=, =s-R=, etc.) —
so invest once, benefit everywhere.

Trigger with =C-.= on a file candidate, then one of the uppercase
letters below. Uppercase keeps these out of the way of Embark's
lowercase defaults (=c= copy, =d= dired, =r= rename, =w= kill, …).

| Key       | Action                                     |
|-----------+--------------------------------------------|
| =L=       | Copy as Org =[[file:...][basename]]= link  |
| =B=       | Copy just the basename                     |
| =A= / =s-|= | Copy the abbreviated path (=~/...=)      |
| =V=       | Reveal in the macOS Finder                 |
| =P=       | Open the parent directory in Dired         |
