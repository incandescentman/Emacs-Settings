...
=~/Library/CloudStorage/Dropbox/...= is normalized to =~/Dropbox/...=
so the two interchangeable macOS paths render identically. Parent
paths longer than =jay/recent-file-parent-column-max= are truncated
from the *left* with a leading ellipsis, so the distinctive tail of
the path (the part closest to the file) stays visible.
