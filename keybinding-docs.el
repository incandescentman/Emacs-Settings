    ...
    (unwind-protect
        (progn
          ...
          (let ((sorted '()))
            ...
            ;; Build reverse lookup: command → list of key bindings (all active maps)
            (let ((cmd-to-keys (make-hash-table :test 'equal)))
              (maphash
               (lambda (mode-cmd-pair _count)
                 (let* ((mode (car mode-cmd-pair))
                        (cmd (cdr mode-cmd-pair))
                        (keys (my/--keys-for-command-in-mode cmd mode mode-buffers)))
                   (when keys
                     (dolist (k keys)
                       (puthash cmd
                                (cons k (gethash cmd cmd-to-keys))
                                cmd-to-keys)))))
               freq-table)
              ...
              (with-current-buffer buf
                ...
                (insert "Bindings are resolved from active keymaps for each recorded major mode (global + local + minor).\n\n")
                ...
                (let ((n 0))
                  (dolist (entry sorted)
                    (when (< n 50)
                      (let* ((count (car entry))
                             (cmd (cdr entry))
                             (keys (my/--sort-keys (gethash cmd cmd-to-keys)))
                             (key-str (if keys
...
(defun my/--mode-buffer-for (mode mode-buffers)
  ...
  (let ((buf (gethash mode mode-buffers)))
    (unless (buffer-live-p buf)
      (setq buf (generate-new-buffer (format " *keyfreq-mode:%s*" mode)))
      (with-current-buffer buf
        (condition-case _err
            (cond
      ...
      (puthash mode buf mode-buffers))
    buf))

(defun my/--normalize-keys (keys)
  "Normalize KEY sequences from `where-is-internal` into strings."
  (when keys
    (let ((out '()))
      (dolist (k keys)
        (let ((desc (key-description k)))
          (when (and desc (> (length desc) 0))
            (push desc out))))
      (delete-dups (nreverse out)))))

(defun my/--sort-keys (keys)
  "Sort KEYS by ergonomics (shorter + simpler first)."
  (when keys
    (let ((uniq (delete-dups (copy-sequence keys))))
      (sort uniq (lambda (a b)
                   (let* ((ra (my/--ergo-rank (my/--key-complexity a)))
                          (rb (my/--ergo-rank (my/--key-complexity b))))
                     (if (/= ra rb)
                         (< ra rb)
                       (< (length a) (length b)))))))))
