...
What I did above works, but it breaks when it deals with punctuation. Example:
...
As you can see, the comma handling gets messed up. Is it possible to fix this, or does the invocation of left-word mess things up? Thanks! 
...
* TODO [#A] a different kind of request                                :rudi:
Hi Rúdi, 
...
* TODO New note about the abbrevs file :rudi:
...
- [ ] separate these out into their own separate abbrevs file, which I can then open-source on Github and share with other Emacs users, and
- [ ] also convert them to TextExpander format, which I can also open-source share with other people.
