...
;;; new version; please remove old one above after testing.
(defun backward-kill-word-correctly ()
  "Kill word."
  (interactive)
  (let ((old-point (point)))
...
**** TODO More follow-up questions                                 :followup:

     - When you say that you want to invoke the command twice and have that
       behaviour, are you expecting it to keep some sort of memory?  Or, on the
       contrary, should the second invocation in your example kill *just* the
       comma regardless of the first invocation?  I will implement it like that
       (no memory) since not only it is the more logical to me, but also it
       should be easily adaptable if you originally intended a different
       semantics.
...
** DONE! :-) Please check if this implementation meets the requirements
...
# In the above case, ideally it should remove the extra space and cycle hyphenation. The cases below are rare enough that I'm not worried about them. ---Jay 

# ^ OK, I have implemented that use case.  It now removes every whitespaces
# inbetween.  Please test.

* DONE! :-) org-clone-subtree-no-time-shift                            :rudi:
...
** DONE! :-) I think this should meet the requirements; please test it
