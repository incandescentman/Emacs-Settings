...
* PLEASE-DEBUG [#A] Parentheses, quotations, and auto-capitalize mode
...
* TODO [#B] check my changes to backward-kill-word-correctly?          :rudi:
...
* TODO [#B] my-delete-backward                                         :rudi:
#+BEGIN_SRC emacs-lisp
...
(defun my/delete-backward ()
  ...
  (save-excursion
    (when (or (looking-at "[[:space:]]")
      ...
      (my/fix-space))))
#+END_SRC
** TODO [#B] my-delete-backward-and-capitalize? 
...
** TODO [#C] my-delete-backward-and-downcase? 
Also, when there's no region, and I'm instead using my-delete-backward to delete backward one character at a time, is it possible to make my-delete-backward detect when I'm deleting a "." and if so, run downcase-word? 

Example: 
: Alice was tired. Instinctively she approached.

Let's say the point is on the blank space after the "." and then I invoke my-delete-backward)
...
* DONE [#A] Extend smart-return for numbered lists
Perfect! Thank you. Can you extend it so that it does this same behavior with numbered lists? So that smart-return runs org-meta-return and takes you to the next line of the numbered list, UNLESS point is at the beginning of the line in which case it escapes the list, same as below? Thanks!
...
** NOTE-TO-JAY Question
   ...
   Could you give an example where it *doesn't* work? In my current setting, it works with bullet lists and numbered ones as well.

** NOTE-TO-RÚDI It works!
You're right, it works! The problem was actually with my smart-return keybinding above. I was using (kbd "RET"). Changing it to (kbd "<return>") fixed the issue. Solved! :-)

* DONE [#B] make smart-punctuation work when point is ON the punctuation?
Is it fairly easy to make smart-punctuation operate when the point is ON the punctuation?

: Alice was hungry,

Let's say the point is ON the comma.
...
Is this easy or will it mess everything else up? Thanks!

** NOTE-TO-JAY Try it
** NOTE-TO-RÚDI Flawless!
...
*** Anyway...
I already tried setting org-blank-before-new-entry to auto:

     ((heading . auto)
     (plain-list-item . auto))

But I think org-blank-before-new-entry works by detecting other blank lines in the area.

So instead here's my idea. I'm going to set org-blank-before-new-entry to this:
...
In other words I think the easiest solution is to set "heading" to "always" and then handle the exceptions manually with an intelligent function.
...
** NOTE-TO-RÚDI it works!
Can you do the same thing for org-insert-todo-heading? That way I can use <M-S-return> to insert TODO headings using the same spacing behavior. Thanks!

** NOTE-TO-JAY Try it again
** NOTE-TO-RÚDI Works perfectly!
