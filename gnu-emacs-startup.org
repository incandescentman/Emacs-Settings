...
;; Do not permit automatic writes until the on-disk table has loaded.
 ...
 (lambda ()
   (condition-case err
       (progn
         ...
         (setq-default abbrev-mode t))
     (error
      (message "Abbrev load failed; automatic abbrev saving remains disabled: %s"
       ...
       (let ((on-macbook (jay/on-macbook-display-p))
         ...
         (if now-fullscreen
             (progn
               ...
               (message "🧭 Fullscreen engaged, sir. Tab bar deployed in ceremonial defense of the notch."))
           (progn
             (tab-bar-mode -1)
             (message "🎩 Fullscreen dismissed. Tab bar has made itself scarce, as propriety demands."))))))))
...
#+BEGIN_SRC emacs-lisp
;; s-k prefix map now defined and populated in keys.el.
;; The old (fset 's-k-prefix s-k-map) here was clobbering keys.el bindings.
#+END_SRC
...
(defun jay/fountain-export-and-open ()
  "Export the current Fountain buffer to PDF via afterwriting and open it."
  (interactive)
  (unless (derived-mode-p 'fountain-mode)
    (user-error "Not a fountain-mode buffer"))
  (unless buffer-file-name
    (user-error "Buffer is not visiting a file"))
  (when (buffer-modified-p) (save-buffer))
  (let* ((source buffer-file-name)
         (pdf (concat (file-name-sans-extension source) ".pdf"))
         (default-directory (file-name-directory source))
         (config (expand-file-name "~/.config/afterwriting/config.json"))
         (cmd (format "afterwriting %s --source %s --pdf %s --overwrite"
                      (if (file-exists-p config)
                          (concat "--config " (shell-quote-argument config))
                        "")
                      (shell-quote-argument source)
                      (shell-quote-argument pdf)))
         (proc (start-process-shell-command
                "fountain-export" "*Fountain Export*" cmd)))
    ;; Stash the PDF path on the process so the sentinel can find it
    ;; without depending on lexical closure capture (this file is
    ;; dynamically bound).
    (process-put proc 'fountain-pdf pdf)
    (message "Exporting → %s" (file-name-nondirectory pdf))
       ...
       (when (eq (process-status p) 'exit)
         (let ((out (process-get p 'fountain-pdf)))
           (if (zerop (process-exit-status p))
               (progn
                 (message "Fountain → %s" out)
                 (call-process "open" nil 0 nil out))
             (message "Fountain export failed (exit %d); see *Fountain Export*"
                      (process-exit-status p))
             (display-buffer "*Fountain Export*"))))))))
...
(defun my/hook (hook)
 ...
 (org-link-set-parameters "hook" :follow #'my/hook)
#+end_src
...
#+BEGIN_SRC emacs-lisp
(defun jay/disable-themes-before-load (&rest _)
 "Disable all active themes before loading a new one."
 (mapc #'disable-theme custom-enabled-themes))
(advice-add 'load-theme :before #'jay/disable-themes-before-load)
#+END_SRC
