...
* TODO [#A] yet another edge case for kill-word-correctly-and-capitalize 
...
* TODO [#B] an edge case for kill-clause
...
* PLEASE-DEBUG [#B] self insert space 
...
* TODO [#A] isearch contents of pasteboard
...
can we implement a way for me to add words to a list of export-to-TextExpander exceptions i.e. words that are >1char that should be exported as case-sensitive, i.be. abbrevationMode 0
* TODO [#C] bold-next-word 
function I can call BEFORE I enter a word that will place * on either side of a word. let's say my keybinding for bold-next-word is M-S-b.

Example:
<KBD><M-S-b>important<SPC)point<SPC>to<SPC>remember</KBD> 
