...
Here's the key bindings cheat sheet in Org-mode format, organized and classified:
...
~s-v~               pasteboard-paste-spaces-maybe
~s-k o m~           org-mode
~s-k f m~           text-mode
~s-k e l~           emacs-lisp-mode
~s-k w c~           wc-mode
~s-k o c~           org-wc-count-subtrees
...
(define-key key-minor-mode-map (kbd "s-A") 'palimpsest-move-region-to-bottom)
...
;; Check if the point is on an image link
 (if (and (thing-at-point 'url)
      (string-match-p "\\(\\.png\\|\\.jpg\\|\\.jpeg\\)$" (thing-at-point 'url)))
   (display-online-image-in-new-buffer)
