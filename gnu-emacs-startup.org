...
(defun org-in-empty-item-p ()
  ...
  (when (org-at-item-p)
    (let ((content (org-trim (buffer-substring-no-properties
                              (line-beginning-position)
                              (line-end-position)))))
      (string-match-p (format "^%s\\s-*$" (regexp-quote (org-list-bullet-string))) content))))
...
(defun org-url-at-point-is-image-p ()
  ...
  (let ((url (thing-at-point 'url)))
    (when url
      (string-match-p (regexp-opt '(".png" ".jpg" ".jpeg" ".gif" ".bmp" ".svg") t) url))))

;; Helper function to check if point is on an Org link
...
(defun smart-return ()
  ...
  (cond
   ...
   ((org-in-empty-item-p)
    ...
    (beginning-of-line)
    (delete-char (length (org-list-bullet-string))))
   ...
   ;; Delete active region and insert newline
   ((use-region-p)
    (delete-region (region-beginning) (region-end))
    (org-return-indent))
