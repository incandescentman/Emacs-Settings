...
*** org delete item
#+begin_src emacs-lisp
(defun org-delete-item ()
 "Delete the current org list item."
 (interactive)
 (when (org-at-item-p)
  (let ((begin (line-beginning-position))
     (end (line-end-position)))
   (delete-region begin (min (1+ end) (point-max)))
...
(defun org-item-empty-p ()
 "Check if the current Org list item is empty."
 (let* ((element (org-element-at-point))
     (contents-begin (org-element-property :contents-begin element))
     (contents-end (org-element-property :contents-end element)))
  (or (null contents-begin)
...
#+BEGIN_SRC emacs-lisp
(defun smart-return ()
  "Smart Return key behavior depending on context."
  (interactive)
  (let* ((context (org-element-context))
    ...
    (cond
     ;; 1. Display image if URL ends with image extensions
     ((and url (string-match-p "\\(\\.png\\|\\.jpg\\|\\.jpeg\\)\\'" url))
      (display-online-image-in-new-buffer url))

     ;; 2. Delete active region and insert newline
     ((use-region-p)
      (delete-region (region-beginning) (region-end))
      (newline))

     ;; 3. Open Org link if org-return-follows-link is enabled
     ((and (eq type 'link) org-return-follows-link)
      (org-open-at-point))

     ;; 4. Exit list when on empty list item
     ((and (org-at-item-p) (org-item-empty-p))
      (org-delete-item)
      (newline))

     ;; 5. Insert new list item
     ((org-at-item-p)
      (org-insert-item (org-element-property :checkbox (org-element-lineage context '(item) t))))

     ;; 6. Prevent stray characters
     ...
     ;; 7. Default org-return behavior in org-mode
     ((derived-mode-p 'org-mode)
      (org-return))

     ;; 8. Fallback to inserting newline
     (t
      (newline)))))
#+END_SRC
