...
(defun kill-clause ()
  ...
  (let ((start-point (point))
        (end-point nil)
        (killed-text ""))
    ;; Search for the end of the clause
    ...
    ;; Kill the region
    (when end-point
      (setq killed-text (buffer-substring start-point end-point))
      (delete-region start-point end-point))

    ;; Handle punctuation and capitalization
    (cond
     ;; If we killed to the end of a sentence
     ((string-match-p "[.!?]\\s-*$" killed-text)
      (when (looking-back "[.!?]\\s-*")
        (delete-backward-char 1))
      (insert " ")
      (capitalize-word 1))
     ;; If we killed to a comma, semicolon, or colon
     ((string-match-p "[,;:]\\s-*$" killed-text)
      (when (looking-back "[,;:]\\s-*")
        (delete-backward-char 2))
      (insert " ")
      (capitalize-word 1))
     ;; If we killed to the end of a line
     (t
      (just-one-space 1)))

    ;; Ensure proper spacing
    (when (looking-at "\\s-+") (delete-horizontal-space))
    (when (looking-back "\\s-+") (delete-horizontal-space t))
