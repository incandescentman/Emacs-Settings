  ...
  (cond (mark-active
        ...
        ((and (org-or-orgtsruct-p)
         ...
         (let ((is-org-chbs (org-checkbox-p))
               (meta-ret-kb (key-binding (kbd "M-RET"))))
           (if meta-ret-kb
               (call-interactively meta-ret-kb)
             (newline))
           (when is-org-chbs
             (insert "[ ] "))))
        ((key-binding (kbd "M-RET")) (call-interactively (key-binding (kbd "M-RET"))))
        ((eq major-mode 'org-mode) (org-return))
        (t (newline))))
