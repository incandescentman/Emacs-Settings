...
(defun pasteboard-paste-adjusted-subtrees-spaces-maybe ()
  "Paste Org text from the system pasteboard, adjust heading levels,
and then remove spaces or smart quotes based on surrounding punctuation."
  (interactive)
  ;; 1) Grab raw text from pbpaste
  (let* ((text (shell-command-to-string "pbpaste"))
         (current-level (save-excursion
                          (if (org-before-first-heading-p)
                              0
    ...
    ;; 2) Shift headings exactly as in your working function
    (setq text (replace-regexp-in-string "\r" "" text))
    (with-temp-buffer
      (insert text)
      (goto-char (point-min))
    ...
    ;; 3) Insert the shifted text right here
    (setq start (point))
    (insert text)
    (setq end (point))

    ;; 4) Now decide: are we next to punctuation -> remove spaces? else remove smart quotes?
    ;;    Since your existing pasteboard-paste-* functions re-call pbpaste, we can't use them directly.
    ;;    Instead, call the sub-steps they do. For instance:
    (let ((prev-char (char-before))
          (next-char (char-after))
          (char-set '(?: ?' ?( ?) ?| ?[ ?] ?/ ?\\ ?\" ?= ?< ?> ?{ ?})))
      (if (or (member prev-char char-set)
              (member next-char char-set))
          ;; We do the "no-spaces" style cleanup
          (my-remove-carriage-returns-in-region start end)
        ;; We do the "no-smart-quotes" style cleanup
        (progn
          (replace-smart-quotes start end)
          (convert-markdown-links-to-org-mode start end)))))
...
(define-key key-minor-mode-map (kbd "s-v") 'pasteboard-paste-spaces-maybe)
(define-key key-minor-mode-map (kbd "s-V") 'pasteboard-paste-adjusted-subtrees-spaces-maybe)
