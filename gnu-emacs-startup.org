...
;; Modern fullscreen toggle that works with native macOS fullscreen
(defun jay/toggle-fullscreen ()
  "Toggle fullscreen mode with proper macOS support."
  (interactive)
  (let ((fullscreen-p (frame-parameter nil 'fullscreen)))
    (set-frame-parameter
     nil 'fullscreen
     (if fullscreen-p nil 'fullboth))
    ;; Adjust font size if you have that function
    (when (fboundp 'jay/adjust-font-size)
      (jay/adjust-font-size))
    ;; Report status
    (message "Fullscreen %s" (if fullscreen-p "disabled" "enabled"))))
