...
(let ((url (thing-at-point 'url)))
  (if (and url (string-match-p "\\(\\.png\\|\\.jpg\\|\\.jpeg\\)$" url))
    (display-online-image-in-new-buffer)
   (message "URL at point: %s" url)
...
(defun smart-return ()
 (interactive)
 (let ((url (thing-at-point 'url)))
  (message "URL at point: %s" url) ; Debug message
  (if (and url (string-match-p "\\(\\.png\\|\\.jpg\\|\\.jpeg\\)$" url))
    (display-online-image-in-new-buffer)
   ...
   (cond
    ...
    (cond
     ((or (looking-back ">")
       ...
       (and url
          (let ((bnds (bounds-of-thing-at-point 'url)))
           (and bnds ; Check if bnds is non-nil
             (or (>= (car bnds) (point))
               (<= (cdr bnds) (point)))))))
     (newline))
     ((char-equal (string-to-char "]") (following-char))
    ...
    ((let ((el (org-element-at-point)))
     (and el
        (eq (car el) 'item)
        (eql (plist-get (cadr el) :contents-begin)
    ...
    ((let ((el (org-element-at-point)))
     (and (not (org--line-empty-p 1))
        el
        (or (memq (car el) '(item plain-list))
          (let ((parent (plist-get (cadr el) :parent)))
           (and parent
             (memq (car parent) '(item plain-list)))))))
    (let ((is-org-chbs (org-checkbox-p)))
     (org-run-like-in-org-mode (lambda () (interactive) (call-interactively 'org-meta-return)))
     (when is-org-chbs
      (insert "[ ] "))))
