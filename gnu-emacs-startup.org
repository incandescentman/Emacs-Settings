...
(define-key key-minor-mode-map (kbd "s-i s-I") 'clone-indirect-buffer-other-window)
...
(define-key key-minor-mode-map (kbd "s-k g a") 'adobe-garamond-pro)
(define-key key-minor-mode-map (kbd "s-k ag") 'adobe-garamond-pro)
(define-key key-minor-mode-map (kbd "s-k gf") 'adobe-garamond-pro)
(define-key key-minor-mode-map (kbd "s-k mf") 'monaco-font)
(define-key key-minor-mode-map (kbd "s-k mo") 'monaco-font)
...
(define-key key-minor-mode-map (kbd "s-k p s") 'org-paste-subtree)
...
(define-key key-minor-mode-map (kbd "s-k p o") 'pomodoro-start-jay)
...
** kill-clause refactored by ChatGPT. Doesn't work
#+begin_EXAMPLE emacs-lisp
(defun kill-clause ()
  (interactive)
  (smart-expand)
  (move-right-if-punctuation)
  (move-left-if-space)
  (handle-asterisk-line)
  (handle-punctuation-and-capitalization)
  (fix-punctuation-issues)
  (fix-multiple-punctuation-marks))

(defun move-right-if-punctuation ()
  (when (or (looking-at ",")
            (looking-at ";")
            (looking-at ":"))
    (right-char 1)))

(defun move-left-if-space ()
  (when (looking-back " ")
    (left-char 1)))

(defun handle-asterisk-line ()
  (if (let ((sm (string-match "\\*+\\s" (thing-at-point 'line))))
        (and sm (= sm 0)))
      (kill-line)
    (let ((old-point (point))
          (kill-punct (my/beginning-of-sentence-p)))
...
(defun handle-punctuation-and-capitalization ()
  (my/fix-space)
  (save-excursion
    (when (my/beginning-of-sentence-p)
      (capitalize-unless-org-heading))))

(defun fix-punctuation-issues ()
  (fix-double-comma)
  (fix-double-colon)
  (fix-colon-comma)
  (fix-comma-period)
  (fix-comma-space-period)
  (fix-space-punct-punct)
  (fix-hyphens)
  (move-cursor-before-punctuation))

(defun fix-double-comma ()
  (when (looking-back "\\, \\, ")
    (new-org-delete-backward-char 2)
    (my/fix-space)))

(defun fix-double-colon ()
  (when (looking-back ":: ")
    (new-org-delete-backward-char 2)
    (my/fix-space)))

(defun fix-colon-comma ()
  (when (looking-back ":, ")
    (left-char 1)
    (delete-backward-char 1)
    (right-char 1)))

(defun fix-comma-period ()
  (when (looking-back "\\,\\. ")
    (left-char 2)
    (delete-backward-char 1)
    (right-char 2)))

(defun fix-comma-space-period ()
  (when (looking-back "\\, \\. ")
    (left-char 2)
    (delete-backward-char 2)
    (right-char 2)))

(defun fix-space-punct-punct ()
  (when (looking-back " \\?")
    (left-char 1)
    (new-org-delete-backward-char 1)
    (right-char 1)))
...
(defun move-cursor-before-punctuation ()
  (when (looking-back "[[:punct:]][[:punct:]]? ")
    (left-char 2))
  (when (looking-back "[[:punct:]][[:punct:]]?")
    (left-char 1)))

(defun fix-multiple-punctuation-marks ()
  (when (or (looking-at ":\\,")
            (looking-at ";\\,")
            (looking-at "\\,\\,")
            (looking-at "\\.\\.")
...
(defun fix-period-comma ()
  (when (looking-at "\\.\\,")
    (delete-forward-char 1)))

(defun fix-colon-period ()
  (when (looking-at ":\\.")
    (delete-forward-char 1)))

(defun delete-space-before-punctuation ()
  (when (and (looking-at "[[:punct:]]\\{1,2\\}")
             (looking-back " "))
    (delete-backward-char 1)))

(defun fix-space-between-punctuations ()
  (when (looking-back "[[:punct:]]\\{1,2\\}")
    (progn
      (forward-char 1)
      (my/fix-space)
      (backward-char 1))))

;; Add the new helper functions to the main function
(defun kill-clause ()
  (interactive)
  (smart-expand)
  (move-right-if-punctuation)
  ...
  (fix-space-between-punctuations))
