...
or in `text-mode`), call `pasteboard-copy-and-replace-em-dashes-in-clipboard`
so that dash sequences are replaced with em dashes.
When working with code (any mode other than `org-mode` or in `org-mode`
with `org-config-files-local-mode` active), call `pasteboard-copy` to copy verbatim."
  (interactive)
...
In prose (in `org-mode` without `org-config-files-local-mode` or in a mode derived from `text-mode`),
always use `pasteboard-paste-without-smart-quotes`.

In code (any mode other than the above, or in `org-mode` when `org-config-files-local-mode` is enabled),
use the existing logic: if the character before or after point is in a predefined set,
...
(define-key key-minor-mode-map (kbd "s-c") 'pasteboard-copy-and-replace-em-dashes-in-clipboard-maybe)
