...
(defun kill-clause ()
  ...
  (if
    ...
    (let ((old-point (point))
      ...
      ;; Modification: Change the regular expression to stop at a period followed by a space or the end of the line
...
;; (get-before-punctuation) ; for some reason this doesn't work here! But works when I run it on its own
)

(defun get-before-punctuation ()
(interactive)
(when
(looking-back "[[:punct:]] ")
(left-char 2)))
...
*** version written by GPT4
#+BEGIN_EXAMPLE emacs-lisp
(defun kill-clause ()
 "Remove a clause or sentence from the current point in the buffer."
 (interactive)
 (smart-expand)

 (if (org-at-heading-p)
   (kill-line)
  (kill-clause-or-sentence))
 (fix-spacing))



(defun kill-clause-or-sentence ()
  "Remove a clause or sentence from the current point in the buffer."
  (let ((start-point (point))
        (kill-punct (beginning-of-sentence-p)))
...
(defun beginning-of-sentence-p ()
 "Check if the point is at the beginning of a sentence."
 (save-excursion
  (skip-chars-backward " \t\n")
  (or (bolp)
...
(defun fix-spacing ()
  (interactive)
 "Fix spacing issues after removing a clause or sentence."
 (let ((spacing-regex-alist
     '((", , " . ", ")
      (":: " . ": ")
      (" \\?" . "?")
      (" , " . ",")
      ...
      (" : " . ":")
      ("----" . "-"))))
  (dolist (spacing-fix spacing-regex-alist)
   (when (looking-back (car spacing-fix))
    (replace-regexp (car spacing-fix) (cdr spacing-fix) nil (point-min) (point))))))
