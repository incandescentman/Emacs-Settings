...
However, I tried to add exceptions for "line-end" (which is not a defined behavior) and also for user-full name. So, can you make it so that capitalize word stops when it finds a line break? Thanks!
