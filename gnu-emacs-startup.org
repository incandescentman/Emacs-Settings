...
(defun pasteboard-paste-spaces-maybe ()
  "Paste from pasteboard, choosing the method based on the current mode.
In prose modes, always remove smart quotes. In shell-script-mode and
emacs-lisp-mode, always use pasteboard-paste-no-spaces. In all other
code modes, use the surrounding-character logic to decide."
  (interactive)
  (cond
   ;; 1) If in Org (without local config) or derived from text-mode → always use `pasteboard-paste-without-smart-quotes`.
   ((or (and (eq major-mode 'org-mode)
             (not (bound-and-true-p org-config-files-local-mode)))
        (derived-mode-p 'text-mode))
    (pasteboard-paste-without-smart-quotes))

   ;; 2) If in shell-script-mode or emacs-lisp-mode → always use `pasteboard-paste-no-spaces`.
   ((or (eq major-mode 'sh-mode)
        (eq major-mode 'emacs-lisp-mode))
    (pasteboard-paste-no-spaces))

   ;; 3) Otherwise → "code fallback" with surrounding-char check.
   (t
