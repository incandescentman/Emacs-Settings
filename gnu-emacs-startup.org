...
emacs-lisp-mode, always use pasteboard-paste-raw. In all other
code modes, use the surrounding-character logic to decide."
  (interactive)
  (cond
   ;; 1) If in Org (without local config) or derived from text-mode → always use `pasteboard-paste-clean`.
   ((or (and (eq major-mode 'org-mode)
             (not (bound-and-true-p org-config-files-local-mode)))
        (derived-mode-p 'text-mode))
    (pasteboard-paste-clean))

   ;; 2) If in shell-script-mode or emacs-lisp-mode → always use `pasteboard-paste-raw`.
