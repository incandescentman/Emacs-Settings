...
In `folder-first' the parent column is padded to a shared width
(clamped between `jay/recent-file-parent-column-min' and
`-max') so basenames line up vertically; parents wider than the column
are left-truncated with a leading ellipsis to keep the deepest, most
distinctive part of the path visible next to the basename.

In `filename-first' the parent has no neighboring column to align
with, so it is rendered as the full path with no padding and no
truncation — overflow is left to Vertico's normal minibuffer
handling. Truncating the head of the path here would hide exactly the
part that disambiguates which filesystem tree the file lives in."
  (let* ((parents
          (mapcar (lambda (p)
       ...
       (if filename-first-p
         ...
         (let* ((shown
                 (if (> (length parent) parent-col)
                   ...
                   parent))
                (parent-pad
                 (make-string (max 0 (- parent-col (length shown))) ?\s)))
           (list base
